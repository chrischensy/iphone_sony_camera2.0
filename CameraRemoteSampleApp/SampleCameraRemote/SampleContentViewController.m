...
- (void)fetchImage:(NSString *)requestURL
{
    ...
    if (urlData) {
        ...
        NSLog(@"SampleContentViewController data object could not be created "
              @"from download URL = %@",
              requestURL);
...
{
    NSLog(@"SampleContentViewController didCameraStatusChanged = %@", status);
    cameraStatus = status;
}

- (void)didFailParseMessageWithError:(NSError *)error
{
    NSLog(@"SampleContentViewController didFailParseMessageWithError error "
          @"parsing JSON string");
    [self openNetworkErrorDialog];
}
                    ...
                    errorMessage:(NSString *)errorMessage
{
    if (resultArray.count > 0 && errorCode < 0) {
        ...
        if (result[@"playbackUrl"] != NULL) {
            ...
            if ([cameraStatus isEqualToString:@"Streaming"]) {
